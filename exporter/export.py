...
def render_health_frontmatter(data: dict) -> str:
    ...
    for metric, v in ((data.get("wyze") or {}).get("body") or {}).items():
        if v is not None and v != "":
            lines.append(f"wyze_{metric}: {v}")
    # Healthy habits pushed to the Worker (/ingest/habits). Written as habit_<name>
    # keys so the dashboard plugin's Habits tab and Dataview pick them up. The Worker
    # already slugs names, but re-sanitize here in case the file came from elsewhere.
    for name, v in sorted(_habits(data).items()):
        lines.append(f"habit_{name}: {v}")
    lines += ["---", "",
...
def render_block(data: dict) -> str:
    ...
    # Wyze body composition (its own line; carried forward when no weigh-in that day).
    wy = data.get("wyze") or {}
    wb = wy.get("body") or {}
    if wb:
        bbits = []
        if wb.get("weight_kg") is not None:
            bbits.append(f"{wb['weight_kg']}kg")
        elif wb.get("weight_lb") is not None:
            bbits.append(f"{wb['weight_lb']}lb")
        for lbl, key, suf in [
            ("", "body_fat_pct", "% fat"), ("muscle", "muscle_mass_kg", "kg"),
            ("water", "body_water_pct", "%"), ("visceral", "visceral_fat", ""), ("BMR", "bmr_kcal", ""),
        ]:
            v = wb.get(key)
            if v is not None:
                bbits.append(f"{lbl} {v}{suf}".strip())
        if bbits:
            suffix = f" (as of {wy.get('measured_date') or '—'})" if wy.get("carried_forward") else ""
            lines.append("**Body comp** " + " · ".join(bbits) + suffix)

    # Healthy habits logged via /ingest/habits (done ones; quantities shown in parens).
    habits = _habits(data)
    done = {n: v for n, v in habits.items() if v}
    if done:
        hbits = [n.replace("_", " ") if v == 1 else f"{n.replace('_', ' ')} ({v:g})" for n, v in sorted(done.items())]
        lines.append("**Habits** " + " · ".join(hbits))
    ...
    field("Wyze-Weight-Kg", wb2.get("weight_kg"))
    field("Wyze-Weight-Lb", wb2.get("weight_lb"))
    field("Wyze-BMI", wb2.get("bmi"))
    field("Wyze-BodyFat-Pct", wb2.get("body_fat_pct"))
    field("Wyze-Muscle-Kg", wb2.get("muscle_mass_kg"))
    field("Wyze-BodyWater-Pct", wb2.get("body_water_pct"))
    field("Wyze-BMR", wb2.get("bmr_kcal"))
    field("Wyze-Visceral", wb2.get("visceral_fat"))
    field("Wyze-BoneMass-Kg", wb2.get("bone_mass_kg"))
    field("Wyze-MetabolicAge", wb2.get("metabolic_age"))
    field("Wyze-Protein-Pct", wb2.get("protein_pct"))

    # Habit fields (explicit 0s included — "didn't do it" is data too).
    for name, v in sorted(habits.items()):
        field("Habit-" + name.replace("_", "-").title(), v)
